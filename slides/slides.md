<!-- .slide: <%= bg("unsplash-iuqmGmst5Po-silh_hiking.jpg") %> id="title" -->
# Accountability <br> with Hope
## 1 Thessalonians 3:6-13
[Mittagberg, Germany. Tobias Mrzyk](https://unsplash.com/photos/iuqmGmst5Po "caption")

>>>
+ Think of s.o. whose **faith** in Chr 
  + you **care** deeply about
+ **Child**/parent, friend/smgp, stud/cowork
+ Might be **averse** to gospel, open/quest, 
  + new believer, or growing
+ Care for their **growth** in Chr:
  + not just **wishful** thinking, but already
+ Shared God's **love** / Word, + want to
  + **Follow-up**: so they put faith into **action**:

---
<!-- .slide: data-background="white" -->
# How can we help others <br> to **grow** in Christ? 
<!-- .element: class="r-fit-text" -->

>>>
+ Not **logistics**: how freq to meet, which curric
  + But: with what **attitude**, purpose, mindset
+ Focus: when **we** are resp to hold another acct
  + any position of **leadership**
+ Like Roman **centurion**: soldiers under him, 
  + and also himself under auth (Mt8:9)
  + Addl layer of appl when on **receiving** end
+ Accountability doesn't have to be **antagonistic**
  + but suffused with **hope** in work of HS
+ Turn to **Scrip**, together behold **Lamb** of God
+ Prayer:

______

+ **Formed** us, **called** to be Your sons+daughters
  + and to walk **worthily** of that calling,
+ Gave us many **promises** as we walk in obedience
  + Planted **faith** in hearts to trust those promises
+ Equip us to **share** that faith w/genuine love
  + to see others **blossom** in faith in You

---
<!-- .slide: <%= bg("unsplash-tcPUbu8H2c0-flowers.jpg") %> -->
## Growing Together, While Apart <br> 當分開時一起成長
<!-- .element: class="r-fit-text" -->

| *1* | Impact of **Faith** | 
|---|---|
| *2a* | Toil of **Love** |
| *2b* | Orphaned but **Loved** |
| *3* | Strength from **Hope** |
| *4* | Growing in Purity + **Love** |
| *5a* | **Hope** of Salvation |
| *5b* | Acts of **Faith** |

>>>
+ Chose 1Thes for **pandemic**: enc. in **FHL**
+ 1H: Thes' **rl** w/Paul + **response** to gospel
  + ch3: crux of 1Thes: **report** from Tim's visit
+ 2H: addr specific **questions** / issues
+ To such encour. psg, might **respond** as such:
+ (1) Paul **happy** with Thes: yes, I love mtg
  + w/**friends** / smgp: **done**, nothing to learn
+ (2) not **realistic**: ppl I deal with
  + not as **receptive** to gospel as Thes
+ (3) not **achievable**: Paul **holy**, I'm not: 
  + can't **relate**. In ch2, Paul+Silas had

---
<!-- .slide: <%= bg("wm-Kerkini_Lake_Port.jpg") %> -->
we endeavored the more **eagerly** and with <br>
**great desire** to see you face to face

[1 Thessalonians 2:17b (ESV)](# "ref")
[Kerkini Lake, Greece. Eimaiokanenas, CC-BY-SA-4.0](https://commons.wikimedia.org/wiki/File:Kerkini_Lake_Port.jpg "caption")

>>>
+ Not just a **social** visit to see friends
  + but heartfelt **concern** for Thes: 
+ **check** up on their faith in Christ

---
<!-- .slide: <%= bg("unsplash-iuqmGmst5Po-silh_hiking.jpg") %> class="outline" id="outline" -->
## Accountability [(1Thes3)](# "ref")

1. **Purpose**: to Strengthen Faith [(v1-5)](# "ref")
1. **Product**: Joy + Thanksgiving [(v6-10)](# "ref")
1. **Prayer**: for Love + Purity [(v11-13)](# "ref")

>>>
+ **Purpose** of send Tim: strengthen
+ **Product** of acct: joy bc stand firm
+ **Prayer** for future: **love** abound
+ **Why** send Tim? bc Thes did **wrong**? No, rather
+ Preached **gospel** in Thes + other cities
  + and endured intense **persec** bc of it
+ Knew would **affect** Thes, new in the faith:
  + "Is **this** what following Christ entails?"
+ This is why sent Tim (v3):

---
<!-- .slide: <%= bg("unsplash-3U7HcqkIbb4-kneel_pray_pew.jpg") %> -->
that no one be **moved** by these afflictions. <br>
For you yourselves know <br>
that we are **destined** for this. 

[1 Thessalonians 3:3 (ESV)](# "ref")
[R. Dr. Manoel Miranda, Brazil. Samuel Martins](https://unsplash.com/photos/3U7HcqkIbb4 "caption")

>>>
+ "**moved**": as dog's **tail** wags unconsc.
  + **swayed** emotionally by ext circum.
+ As child watch **father** / role model break down 
  + under intense **sorrow** / pain:
+ "If even **he** suffers so, what **hope** do I have?"
+ **Suffering** is inherent to Chr life in this age:
+ If procl. **king.** God to world **rejects** His auth,
  + if we **pattern** life after Christ,
+ We should not be **surprised** that we are
  + **destined** (placed) into path of suff as Chr
+ Others observ our **trials**, tempt, failures
+ So **compelled** to send Tim to them (v5)

---
<!-- .slide: <%= bg("unsplash-p0W9Q9gei4g-silhouette.jpg") %> -->
I sent to learn about your **faith**, <br>
for **fear** that somehow <br>
the **tempter** had tempted you <br>
and our labor would be in **vain**.

[1 Thessalonians 3:5 (ESV)](# "ref")
[Alex Jones](https://unsplash.com/photos/p0W9Q9gei4g "caption")

>>>
+ Not **afraid**, but very **real** possib:
+ Amidst suff, Satan **probes** our defenses
  + to **poke holes** in shield of faith
+ Tempt to **give up** on following Chr, or
  + get **angry** / bitter, or
  + **compromise**: neglect God's **calling**
  + chase after **desires** of world
+ **We** are suscep, so are those we **care** for
+ This **danger** is what moved Paul to send Tim
+ A few things to **notice** about how Paul
  + reached out to Thes to strength. their faith:

---
<!-- .slide: <%= bg("unsplash-E0e1NmVclRo-mist_lake.jpg") %> -->
## 1. Strengthen Faith [(v1-5)](# "ref")
<!-- .element: class="r-fit-text" -->

+ A. **Anticipate** Challenges 
+ B. **Initiate** Support 
+ C. Radiate **Comfort**, not Coercion 

<!-- .element: class="r-fit-text" -->

[Anna Goncharova](https://unsplash.com/photos/E0e1NmVclRo "caption")

>>>
+ Paul had prior **experience** suffering for Chr
+ knew **more** was coming, and 
+ **told** Thes in advance (v4):

---
<!-- .slide: <%= bg("unsplash-SrlmVWkVfJM-cross.jpg") %> -->
For when we were with you, <br>
we kept telling you **beforehand** <br>
that we were to **suffer** affliction, <br>
just as it has come to pass

[1 Thessalonians 3:4 (ESV)](# "ref")

>>>
+ not "I **told** you so", but mental **prep**:
  + to **stand** firm in obeying Chr
+ (2) **Initiate** contact: 
  + didn't **wait** for Thes to **ask** for help
  + but pro-actively **sent** support / encour
+ (3) Had auth to **coerce**, but didn't **exer.**:
  + **purpose** of sending Tim was (v2):

---
<!-- .slide: <%= bg("unsplash-C2PvO6dOORY-girls_walk.jpg") %> -->
We sent Timothy ...<br>
to **establish** and **exhort** you in your faith

[1 Thessalonians 3:2 (ESV)](# "ref")
[Legenäsgatan, Sweden. Jon Flobrant](https://unsplash.com/photos/C2PvO6dOORY "caption")

>>>
+ "Establish": **strengthen**, make firm
+ When **weak**: need not more **discip** / work **ethic**
  + but need uds to **equip**, supply, give strength
+ e.g., teach kids' **SS**: frustr. at **immaturity**:
  + Still **growing**: your **task** to teach, show
+ "Exhort" (**enc**): as **hubby**/dad: bring **comfort**
+ How I **respond** to suffering/stress
  + affects how **they** react: if **shaken** or not
+ Do I put on **brave** face, hide pain?
  + Coldly / stoicly demand they **tough** it out
+ Or use **blame** / guilt: "all bc of you!"
  + Shows own **irresponsib** / insecurity
+ Use strength not to coerce, but to **comfort**

---
<!-- .slide: data-background="white" -->
# What is my **aim** when <br> holding others accountable?
<!-- .element: class="r-fit-text" -->

>>>
+ Do I **exert** auth, demand **respect**, instill **fear**?
  + "I **suffered** unjust auth, now you must, too"
+ Rather, drawing **strength** from
  + **faithfulness** of God's Word,
+ **Anticipate** opposition as walk in Chr steps
+ **Initiate** support to help stand **firm**
+ Radiate **comfort**, not coercion
  + **encourage**: call alongside, 
  + walk togeth on **narrow** path leads to life
+ So see how **suff.** can **strengthen** our faith

---
<!-- .slide: <%= bg("unsplash-iuqmGmst5Po-silh_hiking.jpg") %> class="outline" -->
## Accountability [(1Thes3)](# "ref")

1. Purpose: to Strengthen Faith [(v1-5)](# "ref")
1. **Product: Joy + Thanksgiving** [(v6-10)](# "ref")
1. Prayer: for Love + Purity [(v11-13)](# "ref")

>>>
+ Tim **visited** Thes to 
  + **strengthen** + encourage in faith
+ **Returned** to Paul in Athens
+ Reported **result** / product from visit:
+ What happens when account. goes **right**: 
  + v6: Tim proclaimed

---
<!-- .slide: <%= bg("unsplash-bfFX9KZJvhQ-ledger_table.jpg") %> -->
the **good news** of your **faith** and **love** <br>
and reported that you always <br>
**remember** us kindly 

[1 Thessalonians 3:6 (ESV)](# "ref")
[Dim Hou](https://unsplash.com/photos/bfFX9KZJvhQ "caption")

>>>
+ **Gospel**: objectively **good** + benef to hearer
+ **Metric** of success: **faith + love**:
  + **trust** in God's promises, and
  + **love** for one another
+ And didn't **decrease** even in face of **suff.**
+ Had positive **memory** of us, long to **see**
+ So Paul's **reaction** to this good report (v7):

---
<!-- .slide: <%= bg("unsplash-78A265wPiO4-sunlit_valley.jpg") %> -->
in all our **distress** and affliction we have <br>
been **comforted** about you through your faith.

For now we **live**, <br>
if you are **standing** fast in the Lord. 

[1 Thessalonians 3:7-8 (ESV)](# "ref")
[Ciucaș Peak, Romania. David Marcu](https://unsplash.com/photos/78A265wPiO4 "caption")

>>>
+ Amid **suffering** on behalf of gospel, 
  + obtained this **encouragement**: 
  + to know they're still **standing** in Lord
+ Our life, legacy, **motiv.**, defn of success
+ Credit goes to **God**, not us (v9):

---
<!-- .slide: <%= bg("unsplash-pZVi92S-ZMs-hands_raised.jpg") %> -->
For what **thanksgiving** can we return to God <br>
for you, for all the **joy** that we feel <br>
for your sake before our God,

[1 Thessalonians 3:9 (ESV)](# "ref")
[Alex Woods](https://unsplash.com/photos/pZVi92S-ZMs "caption")

>>>
+ Deep-rooted **joy**, not superficial
+ Not contingent on ext. **circum.**
  + or performance of **man**
+ But on inexorable **work of HS**
  + according to God's divine **plan**
+ And the work is not **over** yet!
  + Continually **growing** (v10):

---
<!-- .slide: <%= bg("unsplash-CPPFtCHY6mo-pray_bible.jpg") %> -->
as we **pray** most earnestly night and day <br>
that we may **see** you face to face <br>
and **supply** what is **lacking** in your faith?

[1 Thessalonians 3:10 (ESV)](# "ref")
[Olivia Snow](https://unsplash.com/photos/CPPFtCHY6mo "caption")

>>>
+ Thes' **faith** remained steadfast
  + even though saw **trials** Paul endured
  + and that **they** would soon also endure
+ But still had **more** to learn: 
  + **Lacking** in faith doesn't mean **failure**!
+ Means more **joy** for Paul+Silas:
  + to **equip** + strengthen to face trials
+ **Worth** our efforts, prayers, tears, persever

---
<!-- .slide: data-background="white" -->
# What makes me <br> feel truly **alive**?
<!-- .element: class="r-fit-text" -->

>>>
+ **Motorcycle**! **Sunrise** after 5am run! 
  + **Laughter** w/friends+fam!
  + perhaps **alone** time w/o demands
  + Not **bad** things: **blessings** from God
+ **Promise** from today's passage: can have
  + Abundant **joy**, immeas **thanks** to God,
  + can truly **live**, when
+ See others **standing** firm in Lord,
  + **growing** in **trust** in God's promises
  + despite probing **attacks** of Satan

---
<!-- .slide: <%= bg("unsplash-iuqmGmst5Po-silh_hiking.jpg") %> class="outline" -->
## Accountability [(1Thes3)](# "ref")

1. Purpose: to Strengthen Faith [(v1-5)](# "ref")
1. Product: Joy + Thanksgiving [(v6-10)](# "ref")
1. **Prayer: for Love + Purity** [(v11-13)](# "ref")

>>>
+ Having **rcvd** encour report / good news,
+ What does **future** hold for them?
+ Last section: **prayer** for future of Thes (v11)

---
<!-- .slide: <%= bg("unsplash-74TufExdP3Y-sunlit-road.jpg") %> -->
Now may our God and Father himself, <br>
and our Lord Jesus, **direct our way** to you,

[1 Thessalonians 3:11 (ESV)](# "ref")
[Chuckanut Mtn, WA.  Patrick Fore](https://unsplash.com/photos/74TufExdP3Y "caption")

>>>
+ Don't know where **paths** of life take us
  + But **God** directs them: makes **straight**
+ Earnest **hope** that we will meet again
  + and when we do, we'll see that
+ HS been **working** in hearts in this way (v12):

---
<!-- .slide: <%= bg("unsplash-UOwvwZ9Dy6w-hold_hands.jpg") %> -->
and may the Lord make you <br>
**increase and abound** in love for <br>
one another and for all, as we do for you, 

[1 Thessalonians 3:12 (ESV)](# "ref")
[Nancy, France. Rémi Walle](https://unsplash.com/photos/UOwvwZ9Dy6w "caption")

>>>
+ great in **quantity**, and even **more**: 
  + exceed / overflow: abound in **what**?
+ When I **pray** for loved ones,
  + what do I **desire** for them?
+ That they abound in how many Chr **books** read?
  + free **meals** given to poor?
+ If **hw** done, **house** bought, **illness** healed
  + even **happy** / trouble-free life?
+ Paul+Silas' prayer is they abound in **love**:

---
<!-- .slide: <%= bg("unsplash-YK4N8hd4OL8-paper_heart.jpg") %> -->
## 3. Prayer [(v11-13)](# "ref")

+ A. **Experience** God's Love
+ B. **Exercise** Love to God
+ C. **Exhibit** God's Love Toward Others

<!-- .element: class="r-fit-text" -->

[Klaus Andersen](https://unsplash.com/photos/YK4N8hd4OL8 "caption")

>>>
+ **Experience** God's love (defines love)
+ **Exercise** love to God with all their hearts,
+ **Exhibit** God's love to all around
+ And as God **fills** them with His love,
  + we have a future **hope** (v13):

---
<!-- .slide: <%= bg("unsplash-6tfO1M8_gas-forest_mist_beams.jpg") %> -->
so that he may establish your hearts **blameless** <br>
in **holiness** before our God and Father, <br>
at the **coming** of our Lord Jesus <br>
with all his saints.

[1 Thessalonians 3:13 (ESV)](# "ref")
[Chris Lawton](https://unsplash.com/photos/6tfO1M8_gas "caption")

>>>
+ Chr **returns** soon: faith made **sight**
  + Books **opened**: **deeds** exposed
+ Only by Chr's **grace** we're deemed **blameless**
  + HS **strengthens** us to live in holiness
  + Connect to **ch4**: purity + love
+ Always have **hope**!
+ Old **SS** kids now grown, many **far** from God
  + **infl.** beyond our control, own **decisions**
  + Fear my **son** may become same
+ But **hope** in God, not men: 
  + Chr is the One who **sanctifies**

---
<!-- .slide: data-background="white" -->
# How do I **pray** <br> for others?
<!-- .element: class="r-fit-text" -->

>>>
+ Those who I'm **responsib** for, **watch** my faith
+ Perhaps I don't pray **often**, or mostly for **self**
  + not **strong** / holy enough to care for others
+ or only **perfunctory**, rote
  + or all but **given up** God can **change** them
+ Trust in **God's** power to change hearts
  + Not just to **bless** w/success, comfort
+ But to fill them w/His **love**
  + and **strengthen** hearts for **holy** living
  + until day of His **return**

---
<!-- .slide: <%= bg("unsplash-iuqmGmst5Po-silh_hiking.jpg") %> class="empty" -->

>>>
+ Redeemer + Saviour: as each of us **pictures** 
  + those people You've put in our **hearts**
  + given **resp** + **opp** to share Your Word + life
+ **Thank** You for them: **precious** in Your sight
+ **Give** faith to **strength** their faith in Your prom.
  + well-**prep.** to face **trials** as procl. gospel
+ Use to **comfort**/enc: not succumb **Satan's** schemes
+ When we see them **standing** firm in You, we are
  + truly **alive**, abound **joy**, **thanks** to You
+ May we remem to **pray** fervently for them
  + That You may **fill** them w/Your **love**
  + and **sanctify** hearts until Your return

